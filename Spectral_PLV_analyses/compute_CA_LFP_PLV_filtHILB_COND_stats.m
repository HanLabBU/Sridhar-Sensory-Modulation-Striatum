...
aCOHs=[];aSPs=[];aCOHs2=[];aPHs=[];mean_bs=[];mean_stim=[];mouse_id=[];stim_vect=[];
if stim_freq==1
% 10Hz
...
% Think whether this should be mean or median? Also check which is stim and
% baaseline....and whether done correctly 
mean_bs2=nanmax(allCOHS);
mean_stim2=nanmax(allCOHS2);
mean_bs= [mean_bs, mean_bs2];
mean_stim= [mean_stim, mean_stim2];

mouse_id_sess= ses *ones(1,2*size(allCOHS,2));
mouse_id=[mouse_id,mouse_id_sess];

z=zeros(1,size(allCOHS,2));
stim_vect=[stim_vect,z];
z2=ones(1,size(allCOHS,2));
stim_vect=[stim_vect,z2];

aCOHs= [aCOHs, allCOHS];
aCOHs= [aCOHs, allCOHS2];
%aPHs= [aPHs, allPH1];
end

% Make a table in csv 
aCOHs=aCOHs(~isnan(aCOHs));
aCOHs=aCOHs(~isinf(aCOHs));

stim_vect=stim_vect(~isnan(aCOHs));
stim_vect=stim_vect(~isinf(aCOHs));

mouse_id=mouse_id(~isnan(aCOHs));
mouse_id=mouse_id(~isinf(aCOHs));
%T = table(mouse_id',stim_vect',aCOHs');
varNames = {'MouseID','Stim','PLV'};
T=table(mouse_id',stim_vect',aCOHs','VariableNames',varNames);
...
pheight=160
 ...
 [h,p,ci,stats] = ttest(mean_bs,mean_stim)
 p1=signrank(mean_bs,mean_stim)
savepath='C:\Users\sudiksha\Documents\Codes\Spectral_analysis\Figures\'
 V1=mean_bs';V2=mean_stim';
...
print(gcf, '-dpdf' , '-r300' ,'-painters', [ savepath 'max_box_plot_Ca_LFP_PLV_delta' num2str(stim_freq) '.pdf'])
