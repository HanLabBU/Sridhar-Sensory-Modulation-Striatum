...
percent_median_inc=(median(stim_stat)-median(bs_stat))/ median(bs_stat) *100
percent_mean_inc=(mean(stim_stat)-mean(bs_stat))/ mean(bs_stat) *100
z=zeros(size(bs_stat,2),1);
for i=1:size(bs_stat,2)
    z(i)=(stim_stat(i)-bs_stat(i))/(bs_stat(i))*100
end
...
% freq=freq2.freq;
% save("baseline_speed_power_145Hz.mat",'p_all_sess2','freq')
...
set(gca,'Xtick',[1.2 1.9],'Xticklabel',{'Base','10 Hz'})
%set(gca,'Xtick',[],'Xticklabel',{})
print(gcf, '-dpdf' , '-r300' ,'-painters', [ savepath 'VIOL_mov_peak_spect_10Hz_final.pdf'])
...
freq=freq2.freq;
save("baseline_speed_power_10Hz.mat",'p_all_sess4','freq')
