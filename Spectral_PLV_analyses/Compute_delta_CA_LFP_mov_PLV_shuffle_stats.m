...
aCOHs=[];aSPs=[];aCOHs2=[];aPHs=[];aNT=[];corrLFP=[];mouse_id=[];stim_vect=[];PLV_vect=[];
...
allCOHS=allCOHS(:,Dx>prctile(Dx,calc_transient_perc));
allCOHS2=allCOHS2(:,Dx>prctile(Dx,calc_transient_perc));
PLV= nanmean(allCOHS(frsel,:));
PLV_SH= nanmean(allCOHS2(frsel,:));

PLV_vect= [PLV_vect, PLV];
PLV_vect= [PLV_vect, PLV_SH];
size(PLV_vect)

mouse_id_sess= ses *ones(1,2*size(PLV,2));
mouse_id=[mouse_id,mouse_id_sess];

z=zeros(1,size(PLV,2));
stim_vect=[stim_vect,z];
z2=ones(1,size(PLV,2));
stim_vect=[stim_vect,z2];
end
end

PLV_vect=PLV_vect(~isnan(PLV_vect));
PLV_vect=PLV_vect(~isinf(PLV_vect));

stim_vect=stim_vect(~isnan(PLV_vect));
stim_vect=stim_vect(~isinf(PLV_vect));

mouse_id=mouse_id(~isnan(PLV_vect));
mouse_id=mouse_id(~isinf(PLV_vect));
